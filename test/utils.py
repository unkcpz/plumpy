...
class Saver(object):
    ...
    def _save(self, proc):
        bundle = persistence.Bundle(proc)
        self.snapshots.append(bundle)
        self.outputs.append(proc.outputs.copy())
...
# TODO: behavior seems not right, proc_class is not used.
def check_process_against_snapshots(loop, proc_class, snapshots):  # pylint: disable=unused-argument
    """
    ...
    :rtype: bool
    """
    for _, bundle in zip(list(range(0, len(snapshots))), snapshots):
        ...
        while True:
            ...
            compare_dictionaries(snapshots[-j],
                                 saver.snapshots[-j],
                                 snapshots[-j],
                                 saver.snapshots[-j],
                                 exclude={'exception'})
